"""Persistent preliminary DHW/Pool summary reports for Optima ProLink."""
from __future__ import annotations
import os
from datetime import datetime, timezone
from decimal import Decimal

TABLE_NAME=os.environ.get("NOVARA_SUMMARY_REPORTS_TABLE","NOVARASummaryReports")
_ready=False
...
def ensure_table():
    ...
    try: client.describe_table(TableName=TABLE_NAME)
    except ClientError as exc:
        if (exc.response.get("Error") or {}).get("Code")!="ResourceNotFoundException": raise
        try: client.create_table(TableName=TABLE_NAME,AttributeDefinitions=[{"AttributeName":"SummaryID","AttributeType":"S"}],KeySchema=[{"AttributeName":"SummaryID","KeyType":"HASH"}],BillingMode="PAY_PER_REQUEST")
        except ClientError as ce:
            if (ce.response.get("Error") or {}).get("Code")!="ResourceInUseException": raise
...
def summary_id(lead_id,program): return f"{_text(lead_id)}#{_text(program).upper()}"
...
def save(body):
    ...
    item={"SummaryID":summary_id(lead_id,program),"LeadID":lead_id,"Program":program,"Fields":clean,"UpdatedAt":datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")}
    ensure_table(); _ddb().Table(TABLE_NAME).put_item(Item=item)
    return {"ok":True,"table":TABLE_NAME,"summary":_safe(item)}

def get(lead_id,program):
    lead_id=_text(lead_id); program=_text(program)
    if not lead_id: raise ValueError("leadId is required")
    if program not in ("DHW","Pool"): raise ValueError("program must be DHW or Pool")
    ensure_table(); r=_ddb().Table(TABLE_NAME).get_item(Key={"SummaryID":summary_id(lead_id,program)})
    return {"summary":_safe(r.get("Item")) if r.get("Item") else None}
